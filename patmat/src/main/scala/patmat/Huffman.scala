...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(_.toChar).map(p => (p._1, p._2.length)).toList
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeBits(t: CodeTree, b: List[Bit]): List[Char] =
      t match {
        case Fork(left, right, _, _) =>
          b match {
            case 0 :: bs => decodeBits(left, bs)
            case 1 :: bs => decodeBits(right, bs)
            case _ => throw new IllegalArgumentException()
          }
        case Leaf(char, _) => b match {
          case Nil => List(char)
          case bs => char :: decode(tree, bs)
        }
      }
